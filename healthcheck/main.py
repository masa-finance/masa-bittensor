...
from masa.base.healthcheck import PingMinerSynapse
...
async def ping_uids(dendrite, metagraph, uids, timeout=3):
    ...
    axons = [metagraph.axons[uid] for uid in uids]
    try:
        request = PingMinerSynapse(sent_from=external_ip, is_active=False, version=0)
        ...
        for response in responses:
            ...
            print("-----------------------------------------------------")
        successful_uids = [
            uid for uid, response in zip(uids, responses) if response.is_active
        ]
        failed_uids = [
            uid
...
async def get_miners_versions(subnet_id):
    """
    Get the versions of miners using PingMinerSynapse and return full axons info.
...
async def get_leaderboard(subnet: int):
    try:
        ...
        if not axons:
            return JSONResponse(
                status_code=404,
                content={"message": "No axons found for the given subnet"},
            )
